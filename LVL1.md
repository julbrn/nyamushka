...
- "Масштабируемость вёрстки": большинство блоков незавивисимы, используются переменные, при внесении изменений не нужно менять половину вёрстки

### 😼 верстальщика:

- Пожалуй, для верстальщика важны большинство критериев, указанных выше (семантика, валидность, адаптивность, оптимизация, мастшабиуемость, отсутствие горизонтального скролла и т.д.)

- Код должен быть структурирован и читабелен: адекватное наименование классов (методика БЭМ или др.), инкапсуляция стилей, присутвует форматирование кода (например, prettier, eslint), понятная структура проекта
...
- Чтобы обеспечить масштабирование, максимальную "ре-используемость" и сопровождаемость кода используют такие методолгии именования классов стилей, как SMACSS, ITCSS, БЭМ (я использую БЭМ-методологию)
...
Посмотрю код в devtools и попытаюсь определить, что именно является причиной некорректного отображения. Попробую изменять стили прям там, добавлять префиксы или прописывать код специально для этих браузеров.
...
Если нет возможности уточнить у дизайнера и руководствоваться используемой дизайн-системой, я бы временно сделала базовый эффект, например, повышение насыщенности фона кнопки.
